...
Object.keys(PULSAR_URLS).forEach(function (k) {
  async.retry({times: 20, interval: 10}, function(cb, res) {
    resolve(PULSAR_URLS[k], function(err) {
      if (err) {
        log.log("error", "Error on initial resolve for", PULSAR_URLS[k]);
      }
      cb(err);
    });
  }, function(err, res) {
...
async.retry({times: 20, interval: 10}, function(cb, res) {
  resolve(ADMIN_URL, function(err) {
    if (err) {
      log.log("error", "Error on initial resolve for", ADMIN_URL);
    }
    cb(err);
  });
}, function(err, res) {
  if (err) {
    log.log("error", "Failed initial resolve for", ADMIN_URL);
  }
});
