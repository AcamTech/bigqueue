...
var ALLOWED_CLUSTERS = ['notifications', 'default'];

var PULSAR_FULLY_MIGRATED_CLUSTERS = ["notifications"];

if ((process.env.SCOPE && process.env.SCOPE.indexOf("webnode") != -1) || (process.env.SERV && process.env.SERV.indexOf("web_node") != -1)) {
  Object.keys(PULSAR_URLS).forEach(function (k) {
    if (process.env.SCOPE.indexOf(k) != -1) {
      async.retry({times: 30, interval: 10}, function(cb, res) {
        resolve(PULSAR_URLS[k], function(err) {
          if (err) {
            log.log("error", "Error on initial resolve for", PULSAR_URLS[k], err);
          }
          cb(err);
        });
      }, function(err, res) {
        if (err) {
          log.log("error", "Failed initial resolve for", PULSAR_URLS[k], err);
        } else {
          log.log("error", "Successful initial resolve for", PULSAR_URLS[k], err);
        }
      });
    }
  });
...
function publish(cluster, msg, callback) {
  if (ALLOWED_CLUSTERS.indexOf(cluster) != -1) {
    ...
    resolve(url, function(err, addr) {
      if (err) {
        log.log("error", "Error resolving dns", err);
        return callback(err);
      }
      if (!addr) {
...
function createConsumer(topic, consumer, cluster, callback) {
  if (ALLOWED_CLUSTERS.indexOf(cluster) != -1) {
    var enabled = false;
    if (FULLY_MIGRATED_CLUSTERS.indexOf(cluster) != -1) {
      enabled = true;
    }
    resolve(ADMIN_URL, function(err, addr) {
      if (err) {
...
module.exports = {
  ...
  getTopic: getTopic,
  FULLY_MIGRATED_CLUSTERS: PULSAR_FULLY_MIGRATED_CLUSTERS,
