...
BigQueueClusterClient.prototype.getMessage = function(topic,group,visibilityWindow,callback){
    ...
    this.withSomeClient(
        ...
        function(err,data, workClient){
          ...
          statsDClient.increment("application.bigqueue.consumers.stats.get_messages",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
          if(data === undefined) {
            statsDClient.increment("application.bigqueue.consumers.stats.get_messages_empty",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
          } else {
            statsDClient.increment("application.bigqueue.consumers.stats.get_messages_with_content",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
          }
          if(err){
              statsDClient.increment("application.bigqueue.consumers.stats.get_message_errors",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
...
BigQueueClusterClient.prototype.ackMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.ackMessage(topic,group,repData.id,function(err){
        ...
        try {
          statsDClient.increment("application.bigqueue.consumers.stats.ack_messages",1, ["topic:"+topic, "consumer:"+group,"cluster:"+self.cluster]);
        }catch(e) {
          log.error("Error on ack message: "+e);
        }
        if(err){
            statsDClient.increment("application.bigqueue.consumers.stats.ack_message_errors",1, ["cluster:"+self.cluster,"topic:"+topic, "consumer:"+group]);
...
BigQueueClusterClient.prototype.failMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.failMessage(topic,group,repData.id,function(err){
        ...
        statsDClient.increment("application.bigqueue.consumers.stats.fail_message_action",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group,"cluster_node:"+node.id]));
        if(err){
            err = {"msg":err}
