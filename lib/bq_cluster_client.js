    ...
    this.withSomeClient(
        ...
        function(err,key, workClient) {
            ...
            if (typeof message.msg === 'string') {
                jsdog.recordSimpleMetric("application.bigqueue.topics.stats.message_size", Buffer.byteLength(message.msg, 'utf-8'), self.baseTags.concat(["topic:"+topic]));
            }
...
BigQueueClusterClient.prototype.ackMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.ackMessage(topic,group,repData.id,function(err){
        ...
        try {
          jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.ack_messages",1, ["topic:"+topic, "consumer:"+group,"cluster:"+self.cluster]);
        }catch(e) {
          log.error("Error on ack message: "+e);
        }
        if(err){
            jsdog.recordSimpleMetric("application.bigqueue.consumers.stats.ack_message_errors",1, ["cluster:"+self.cluster,"topic:"+topic, "consumer:"+group]);
