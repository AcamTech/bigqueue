...
BigQueueClusterClient.prototype.postMessage = function(topic,message,callback){
    ...
    this.withSomeClient(
        ...
        function(err,key, workClient) {
            statsDClient.increment("application.bigqueue.topics.stats.create_messages_2.sum",1, self.baseTags.concat(["topic:"+topic]));
...
BigQueueClusterClient.prototype.ackMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.ackMessage(topic,group,repData.id,function(err){
        ...
        try {
          statsDClient.increment("application.bigqueue.consumers.stats.ack_messages_2.sum",1, ["topic:"+topic, "consumer:"+group,"cluster:"+self.cluster]);
        }catch(e) {
