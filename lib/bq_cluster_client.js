...
BigQueueClusterClient.prototype.withSomeClient = function(run,cb){
    ...
    function monitor(){
        ...
        setTimeout(function(){
            if(!m.emited){
                statsDClient.gauge("application.bigqueue.web_node.timeouts.sum",1, self.baseTags.concat(["cluster_node:"+actualClient.id]));
...
BigQueueClusterClient.prototype.postMessage = function(topic,message,callback){
    ...
    this.withSomeClient(
        ...
        function(err,key, workClient) {
            statsDClient.gauge("application.bigqueue.topics.stats.create_messages.sum",1, self.baseTags.concat(["topic:"+topic]));
            ...
            statsDClient.gauge("application.bigqueue.cluster_node.stats.create_messages.sum",1, self.baseTags.concat(["cluster_node:"+(workClient ? workClient.id : "error")]));
            if(err) {
              statsDClient.gauge("application.bigqueue.topics.stats.create_message_errors.sum",1, self.baseTags.concat(["topic:"+topic]));
            }
...
BigQueueClusterClient.prototype.getMessage = function(topic,group,visibilityWindow,callback){
    ...
    this.withSomeClient(
        ...
        function(err,data, workClient){
          ...
          statsDClient.gauge("application.bigqueue.consumers.stats.get_messages.sum",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
          if(data === undefined) {
            statsDClient.gauge("application.bigqueue.consumers.stats.get_messages_empty.sum",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
          } else {
            statsDClient.gauge("application.bigqueue.consumers.stats.get_messages_with_content.sum",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
          }
          if(err){
              statsDClient.gauge("application.bigqueue.consumers.stats.get_message_errors.sum",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
...
BigQueueClusterClient.prototype.ackMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.ackMessage(topic,group,repData.id,function(err){
        ...
        try {
          statsDClient.gauge("application.bigqueue.consumers.stats.ack_messages.sum",1, ["topic:"+topic, "consumer:"+group,"cluster:"+self.cluster]);
        }catch(e) {
          log.error("Error on ack message: "+e);
        }
        if(err){
            statsDClient.gauge("application.bigqueue.consumers.stats.ack_message_errors.sum",1, ["cluster:"+self.cluster,"topic:"+topic, "consumer:"+group]);
...
BigQueueClusterClient.prototype.failMessage = function(topic,group,recipientCallback,callback){
    ...
    node.client.failMessage(topic,group,repData.id,function(err){
        ...
        statsDClient.gauge("application.bigqueue.consumers.stats.fail_message_action.sum",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group,"cluster_node:"+node.id]));
        if(err){
            err = {"msg":err}
