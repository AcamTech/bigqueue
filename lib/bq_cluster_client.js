var fs = require('fs'),
    ...
    TOPIC_STRUCT_REFRESH_INTERVAL = 20*1000, // 1 minute
...
function BigQueueClusterClient(createClientFunction,
    ...
    self.refreshClusterStructInterval = undefined;
    self.clearStatsInterval = undefined;
...
BigQueueClusterClient.prototype.startTimers = function(){
  var self = this;
  self.clearStatsInterval = setInterval(function() {
    self.createMessagesStats = {};
    self.getMessagesStats = {};
  }, STATS_CLEAR_INTERVAL);

  var refreshTopicStruct = function(cb){
    try {
      needle.get(self.topicsStructUrl,{read_timeout:500}, function(err, response) {
        if(err || response.statusCode != 200) {
          log.log("error","Error getting topics ["+self.topicsStructUrl+"]",err)
        } else {
          var clusterTopics = {}
          if(response && response.body && response.body.topics) {
  ...
  refreshTopicStruct(function() {
    process.nextTick(function() {
      self.emit("ready");
    });
  })
  self.refreshClusterStructInterval = setInterval(refreshTopicStruct,self.topicStructRefreshInterval);
}
...
BigQueueClusterClient.prototype.shutdown = function(){
    ...
    if(this.refreshClusterStructInterval) {
      clearInterval(this.refreshClusterStructInterval);
    }
...
BigQueueClusterClient.prototype.nodeAdded = function(node){
    ...
    if(actualCluster) {
      var oldClient = actualCluster["client"];
...
BigQueueClusterClient.prototype.healthcheck = function(cb){
    if(this.nodes.length == 0){
      ...
      cb({msg:"Client has not Loaded yet"})
    } else {
      cb()
    }
}
...
BigQueueClusterClient.prototype.postMessage = function(topic,message,callback){
    ...
    if(this.clusterTopics[topic] != undefined) {
      ...
      if( this.clusterTopics[topic].max_new_messages_per_second &&
          this.clusterTopics[topic].max_new_messages_per_second > 0 &&
          this.clusterTopics[topic].max_new_messages_per_second - this.createMessagesStats[topic] < 0) {
        ...
        statsDClient.increment("application.bigqueue.topics.stats.create_messages_reject_by_limit_exceeds.sum",
                                (this.clusterTopics[topic].max_new_messages_per_second - this.createMessagesStats[topic])*-1,
        ...
        return callback({msg:"High Traffic detected on topic "+
                        ...
                        "["+this.clusterTopics[topic].max_new_messages_per_second+" permited]"
                        , limit_rate: true})
...
BigQueueClusterClient.prototype.getMessage = function(topic,group,visibilityWindow,callback){
    ...
    if(this.getMessagesStats[topic+"-"+group] == undefined) {
      this.getMessagesStats[topic+"-"+group] = 0;
    }
    this.getMessagesStats[topic+"-"+group]++;

    if(this.clusterTopics[topic] != undefined && this.clusterTopics[topic].consumers[group] != undefined) {
      if(!this.clusterTopics[topic].consumers[group].enabled) {
        statsDClient.increment("application.bigqueue.consumers.stats.get_messages_reject_by_no_enabled.sum",1, self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
        return callback({msg:"Consumer is Disabled, contact melicloud@ or noc@ to check it", consumer_disabled:true})
      }
      if(this.clusterTopics[topic].consumers[group].max_get_messages_per_second &&
          this.clusterTopics[topic].consumers[group].max_get_messages_per_second > 0 &&
          this.clusterTopics[topic].consumers[group].max_get_messages_per_second - this.getMessagesStats[topic+"-"+group] < 0){
        statsDClient.increment("application.bigqueue.consumers.stats.get_messages_reject_by_limit_exceeds.sum",
                                (this.clusterTopics[topic].consumers[group].max_get_messages_per_second - this.getMessagesStats[topic+"-"+group])*-1,
                                self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
        statsDClient.increment("application.bigqueue.consumers.stats.get_messages_reject_by_limit.sum",1,
                                self.baseTags.concat(["topic:"+topic, "consumer:"+group]));
        return callback({msg:"High Traffic detected on consumer "+
                        "["+this.getMessagesStats[topic+"-"+group]+" request]"+
                        "["+this.clusterTopics[topic].consumers[group].max_get_messages_per_second+" permited]"
                        , limit_rate: true})
        return callback({msg:"Consumer is Disabled, contact melicloud@ or noc@ to check it", consumer_disabled:true})
      }
    }
...
BigQueueClusterClient.prototype.getConsumerType = function(topic, consumer) {
  try {
    if (this.clusterTopics && this.clusterTopics[topic] != undefined && this.clusterTopics[topic].consumers[consumer] != undefined) {
      return this.clusterTopics[topic].consumers[consumer]['consumer_type'];
    }
    return undefined;
  } catch (e) {
    log.log("error", "Error failed to get consumer type", err);
    return undefined;
  }
}
