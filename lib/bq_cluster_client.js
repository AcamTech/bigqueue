    ...
    if(this.clusterTopics[topic] != undefined) {
      ...
      if(this.clusterTopics[topic].max_new_messages_per_seccond > 0 && this.clusterTopics[topic].max_new_messages_per_seccond - this.createMessagesStats[topic] < 0) {
        ...
        statsDClient.increment("application.bigqueue.topics.stats.create_messages_reject_by_limit_exceeds.sum",
                                (this.clusterTopics[topic].max_new_messages_per_seccond - this.createMessagesStats[topic])*-1,
...
BigQueueClusterClient.prototype.getMessage = function(topic,group,visibilityWindow,callback){
    ...
    if(this.clusterTopics[topic] != undefined && this.clusterTopics[topic].consumers[group] != undefined) {
      ...
      if(this.clusterTopics[topic].consumers[group].max_get_messages_per_seccond > 0 &&
          this.clusterTopics[topic].consumers[group].max_get_messages_per_seccond - this.getMessagesStats[topic+"-"+group] < 0){
        statsDClient.increment("application.bigqueue.consumers.stats.get_messages_reject_by_limit_exceeds.sum",
                                (this.clusterTopics[topic].consumers[group].max_get_messages_per_seccond - this.getMessagesStats[topic+"-"+group])*-1,
        ...
        return callback({msg:"High Traffic detected on consumer "+
                        ...
                        "["+this.clusterTopics[topic].consumers[group].max_get_messages_per_seccond+" permited]"
                        , limit_rate: true})
        return callback({msg:"Consumer is Disabled, contact melicloud@ or noc@ to check it", consumer_disabled:true})
      }
    }
