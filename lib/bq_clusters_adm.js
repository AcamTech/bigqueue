...
BigQueueClustersAdmin.prototype.getEndpoinsForCluster = function(cluster, callback) {
  this.mysqlPool.query("SELECT host, port, description FROM endpoints WHERE cluster = ?", [cluster],function(err,data) {
    callback(err, data);
  });
...
BigQueueClustersAdmin.prototype.listClusters = function(callback){
    mysqlCriticPool.query("SELECT name FROM clusters", function(err, data) {
     var clusters = [];
      if(err) {
        callback(err);
      } else {
...
BigQueueClustersAdmin.prototype.getNodesByCluster = function(cluster, callback) {
    ...
    mysqlCriticPool.query("SELECT dn.id id, dn.host host, dn.port port, dn.status status, dn.type type, dn.options options, nj.journal_id journal "+
                        "FROM data_nodes dn " +
...
BigQueueClustersAdmin.prototype.getNodeDataByType = function(cluster,node,type,callback){
  ...
  async.series([
    function(cb) {
    mysqlCriticPool.query("SELECT id, host, port, status, options FROM data_nodes WHERE type = ? and cluster = ? and id = ?", [type, cluster, node], function(err,data) {
         if(err) {
           return cb(err);
         }
         if(data.length != 1) {
      ...
      function(cb) {
        if(type == "node") {
          mysqlCriticPool.query("SELECT journal_id FROM node_journals WHERE node_id = ?", [node], function(err, data) {
...
BigQueueClustersAdmin.prototype.updateNodeMetrics = function(cluster,node,stats, callback) {
  ...
  async.each(stats.topics_stats, function(topic_stats, tCb) {
    ...
    async.each(topic_stats.consumers, function(consumer,cCb) {
      ...
      //Store into db
      this.mysqlPool.query("UPDATE stats SET lag = ?, fails = ?, processing = ?, last_update = ? WHERE cluster = ? and node = ? and topic = ? and consumer = ?",
        [stats.lag, stats.fails, stats.processing, sample_data,
          ...
          if(res.affectedRows > 0) {
            ...
            this.mysqlPool.query("INSERT INTO stats (cluster, node, topic, consumer, lag, fails, processing, last_update) VALUES (?, ?, ? ,? ,?, ? ,? ,?) ",
