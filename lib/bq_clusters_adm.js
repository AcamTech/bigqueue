var events = require("events"),
    ...
    METRICS_INTERVAL = 10,
    METRICS_PUSHERS = 5,
    METRICS_PUSHER_MAX_BATCH = 10,
    METRICS_PUSHER_MAX_QUEUE_SIZE = 50,
    DEFAULT_MAX_NEW_MESSAGES_PER_SECCOND = 5,
    DEFAULT_MAX_GET_MESSAGES_PER_SECCOND = -1
    DEFAULT_CONSUMER_TYPE = "native"
    PULSAR_CONSUMER_TYPE = "pulsar"
    PULSAR_FULLY_MIGRATED_CLUSTERS = ["notifications"];
...
BigQueueClustersAdmin.prototype.createTopic = function(topic,callback){
 ...
 topic["enabled"] = topic["enabled"] ? 1 : 0;
 if(topic["max_new_messages_per_second"] == undefined) {
   topic["max_new_messages_per_second"] = DEFAULT_MAX_NEW_MESSAGES_PER_SECCOND;
 }
 ...
 this.mysqlPool.getConnection(function(err, mysqlConn) {
    ...
    if(topic.cluster) {
      mysqlConn.query("SELECT name FROM clusters where name = ?",[topic.cluster], function(err, data) {
       if(err) {
        ...
        cb({"err":"Cluster "+topic.cluster+" not found", not_found: true});
       } else {
  ...
  function(cb) {
    mysqlConn.query("INSERT INTO topics (topic_id, tenant_id, tenant_name, topic_name, cluster, ttl, create_time, enabled, max_new_messages_per_second) VALUES (?,?,?,?,?,?, now(),?,?)",
                   [id, topic.tenant_id, topic.tenant_name, topic.name, cluster, topic.ttl,topic.enabled,topic.max_new_messages_per_second], cb);
  },
  function(cb) {
...
BigQueueClustersAdmin.prototype.createConsumerGroup = function(consumer,callback){
  ...
  if(consumer["enabled"] == undefined) {
    consumer["enabled"] = true;
  }
  consumer.consumer_type = consumer.consumer_type || DEFAULT_CONSUMER_TYPE;
  consumer.max_get_messages_per_second = consumer.max_get_messages_per_second || DEFAULT_MAX_GET_MESSAGES_PER_SECCOND;
  consumer["enabled"] = consumer["enabled"] ? 1 : 0;
    ...
    async.series([
      function(cb) {
       ...
       self.getTopicData(consumer.topic_id,function(err, data) {
         ...
         if (PULSAR_FULLY_MIGRATED_CLUSTERS.indexOf(cluster) != -1) {
           consumer.consumer_type = PULSAR_CONSUMER_TYPE;
         }
         cb(err, data);
       });
      },
      ...
      function(cb) {
        ...
        mysqlConn.query("INSERT INTO consumers (consumer_id, tenant_id, tenant_name, consumer_name, cluster, topic_id, create_time, enabled,max_get_messages_per_second, consumer_type) VALUES (?,?,?,?,?,?,now(),?,?,?)",
                         [id, consumer.tenant_id, consumer.tenant_name, consumer.name, cluster, consumer.topic_id,consumer.enabled,consumer.max_get_messages_per_second, consumer.consumer_type], cb);
      },
      function(cb) {
...
BigQueueClustersAdmin.prototype.getTopicDataByCriteria = function(criteria, callback) {
  ...
  var topics_idx={};
  var query = "SELECT topic_id, ttl, cluster, enabled, max_new_messages_per_second FROM topics t "
    ...
    async.waterfall([
      ...
      function(topics_db, cb) {
        ...
        topics_db.forEach(function(topic) {
          var data = {topic_id: topic.topic_id, cluster: topic.cluster,
            ttl: topic.ttl, enabled: topic.enabled == 1 ? true: false, max_new_messages_per_second: topic.max_new_messages_per_second,
            consumers: []};
          topics.push(data);
        ...
        self.getConsumerByCriteria(criteria, function(err, data) {
          ...
          data.forEach(function(consumer) {
            ...
            if(topic_data) {
              topic_data.consumers.push({consumer_id: consumer.consumer_id,
                enabled: consumer.enabled,
                max_get_messages_per_second: consumer.max_get_messages_per_second,
                consumer_stats: consumer.consumer_stats
              });
            } else {
...
BigQueueClustersAdmin.prototype.getClusterStruct = function(cluster, callback) {
  var query = "SELECT t.topic_id t_topic_id, "+
    "t.tenant_id t_tenant_id, "+
    "t.tenant_name t_tenant_name, "+
    "t.topic_name t_topic_name, "+
    "t.ttl t_ttl, "+
    "t.enabled t_enabled, "+
    "t.max_new_messages_per_second t_max_new_messages_per_second, "+
    "t.create_time t_create_time, "+
    "c.consumer_id c_consumer_id, "+
    "c.enabled c_enabled, "+
    "c.tenant_id c_tenant_id, "+
    "c.tenant_name c_tenant_name, "+
    "c.consumer_name c_consumer_name, "+
    "c.max_get_messages_per_second c_max_get_messages_per_second, "+
    "c.consumer_type c_consumer_type, "+
    "c.create_time c_create_time "+
    "FROM topics t LEFT OUTER JOIN consumers c ON t.topic_id = c.topic_id "+
    "WHERE t.cluster=?";
  this.mysqlPool.getConnection(function(err, mysqlConn) {
    if(err) {
      if(mysqlConn) {
    ...
    mysqlConn.query(query, [cluster], function(err, data) {
      mysqlConn.release();
      if(err) {
        return callback(err);
      }
      ...
      data.forEach(function(e) {
        if(topics[e["t_topic_id"]] == undefined) {
          topics[e["t_topic_id"]] = {
            "topic_id": e["t_topic_id"],
            "tenant_id": e["t_tenant_id"],
            "tenant_name": e["t_tenant_name"],
            "topic_name": e["t_topic_name"],
            "ttl": e["t_ttl"],
            "enabled": e["t_enabled"] == 1 ? true: false,
            "max_new_messages_per_second": e["t_max_new_messages_per_second"],
            "create_time": e["t_create_time"],
            "consumers": []
          }
        }
        if(e["c_consumer_id"]) {

          topics[e["t_topic_id"]].consumers.push({
            "consumer_id": e["c_consumer_id"],
            "enabled": e["c_enabled"] == 1 ? true: false,
            "tenant_id": e["c_tenant_id"],
            "tenant_name": e["c_tenant_name"],
            "consumer_name": e["c_consumer_name"],
            "create_time": e["c_create_time"],
            "consumer_type": e["c_consumer_type"],
            "max_get_messages_per_second": e["c_max_get_messages_per_second"]
          })
        }
      })
      var response = [];
      Object.keys(topics).forEach(function(k) {
        response.push(topics[k]);
      });
...
BigQueueClustersAdmin.prototype.getConsumerByCriteria = function(criteria, callback) {
  ...
  var query = "SELECT c.consumer_id consumer_id, c.topic_id topic_id, "+
   "c.cluster cluster, "+
   "sum(lag) as lag, "+
   "sum(fails) as fails, sum(processing) as processing, c.enabled, c.max_get_messages_per_second,c.consumer_type "+
   "FROM consumers c " +
   "LEFT OUTER JOIN stats s "+
   "ON(s.consumer = c.consumer_id AND s.topic = c.topic_id) ";
  var query_criteria = getCriteriaQuery(criteria,"c.");
  query+=query_criteria.query;
  query+=" GROUP BY c.consumer_id , c.topic_id, c.cluster,c.enabled, c.max_get_messages_per_second,c.consumer_type ";
  this.mysqlCriticPool.getConnection(function(err, mysqlConn) {
    if(err) {
      if(mysqlConn) {
        mysqlConn.release();
      ...
      data.forEach( function(e){
        if(e.consumer_id)
          consumers.push({topic_id: e.topic_id, consumer_id: e.consumer_id, ttl: e.ttl,
                         cluster: e.cluster,
                         enabled: e.enabled == 1 ? true: false, max_get_messages_per_second: e.max_get_messages_per_second,
    ...
    async.series([
      function(cb) {
        if((data && data.length != 1) || !data) {
          cb({msg: "Consumer ["+consumer+"] for topic ["+topic+"] not found",not_found: true});
        } else {
...
BigQueueClustersAdmin.prototype.updateNodeMetricsAsync = function() {
  ...
  function doNodeMetrics() {
    var q = [];
    var e = undefined, c = 0;
    //Retrieve metrics up to METRICS_PUSHER_MAX_BATCH
    while((e = self.asyncStatsQueue.shift()) && c < METRICS_PUSHER_MAX_BATCH) {
      q.push(e)
      c++;
    }
    jsdog.recordSimpleMetric("application.bigqueue.admin_api.pusher_queue_size", q.length);
    if(q.length > 0) {
            ...
            async.eachSeries(q, function(stat, cb) {
              ...
              async.each(topic_stats, function(topic_stats, tCb) {
                ...
                async.each(topic_stats.consumers, function(consumer,cCb) {
                  ...
                  connection.query("SELECT consumer_type from consumers where topic_id = ? AND consumer_id = ?",
                    [topic_stats.topic_id, consumer.consumer_id], function (err, data) {
                      if (err || (data.length > 0 && data[0]['consumer_type'] != 'pulsar')) {
                        try {
  ...
  /*
   * Every function recall theirself after finish because that on an increasing calls
   * It could start to increase the queue exponentially because of that to have multiple functions running
   * could help to lead with this problem
   */
  for(var i=0; i<METRICS_PUSHERS; i++) {
    doNodeMetrics();
  }
}
...
BigQueueClustersAdmin.prototype.updateNodeMetrics = function(cluster,node,stats, callback) {
  ...
  if(self.asyncStatsQueue.length>=METRICS_PUSHER_MAX_QUEUE_SIZE) {
    self.asyncStatsQueue.pop();
  }
  self.asyncStatsQueue.push({"cluster":cluster,
                          "node":node,
                          "topic_stats":stats.topics_stats,
                          "sample_date": new Date(stats.sample_date)});
  jsdog.recordSimpleMetric("application.bigqueue.admin_api.pusher_queue_waiting_size", self.asyncStatsQueue.length);
