...
BigQueueClustersAdmin.prototype.updateNodeMetricsAsync = function() {
  ...
  function doNodeMetrics() {
    ...
    while((e = self.asyncStatsQueue.pop()) && c < METRICS_PUSHER_MAX_BATCH) {
      q.push(e)
      c++;
    }
...
BigQueueClustersAdmin.prototype.updateNodeMetrics = function(cluster,node,stats, callback) {
  ...
  if(self.asynStatsQueue.length>=METRICS_PUSHER_MAX_QUEUE_SIZE) {
  ...
  jsdog.recordSimpleMetric("application.bigqueue.admin_api.pusher_queue_waiting_size", self.asynStatsQueue.length);
