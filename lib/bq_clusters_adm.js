...
BigQueueClustersAdmin.prototype.updateNodeMetrics = function(cluster,node,stats, callback) {
  ...
  async.each(stats.topics_stats, function(topic_stats, tCb) {
    ...
    async.each(topic_stats.consumers, function(consumer,cCb) {
      ...
      //Store into db
      self.mysqlPool.query("UPDATE stats SET lag = ?, fails = ?, processing = ?, last_update = ? WHERE cluster = ? and node = ? and topic = ? and consumer = ?",
        [stats.lag, stats.fails, stats.processing, sample_data,
          ...
          if(res.affectedRows > 0) {
            ...
            self.mysqlPool.query("INSERT INTO stats (cluster, node, topic, consumer, lag, fails, processing, last_update) VALUES (?, ?, ? ,? ,?, ? ,? ,?) ",
