...
BigQueueClustersAdmin.prototype.createTopic = function(topic,callback){
 ...
 if(topic["max_new_messages_per_seccond"] == undefined) {
   topic["max_new_messages_per_seccond"] = DEFAULT_MAX_NEW_MESSAGES_PER_SECCOND;
 }
 ...
 this.mysqlPool.getConnection(function(err, mysqlConn) {
  ...
  function(cb) {
    mysqlConn.query("INSERT INTO topics (topic_id, tenant_id, tenant_name, topic_name, cluster, ttl, create_time, enabled, max_new_messages_per_seccond) VALUES (?,?,?,?,?,?, now(),?,?)",
                   [id, topic.tenant_id, topic.tenant_name, topic.name, cluster, topic.ttl,topic.enabled,topic.max_new_messages_per_seccond], cb);
  },
  function(cb) {
...
BigQueueClustersAdmin.prototype.createConsumerGroup = function(consumer,callback){
  ...
  consumer.max_get_messages_per_seccond = consumer.max_get_messages_per_seccond || DEFAULT_MAX_GET_MESSAGES_PER_SECCOND;
  ...
  self.mysqlPool.getConnection(function(err, mysqlConn) {
    ...
    async.series([
      ...
      function(cb) {
        ...
        mysqlConn.query("INSERT INTO consumers (consumer_id, tenant_id, tenant_name, consumer_name, cluster, topic_id, create_time, enabled,max_get_messages_per_seccond) VALUES (?,?,?,?,?,?,now(),?,?)",
                         [id, consumer.tenant_id, consumer.tenant_name, consumer.name, cluster, consumer.topic_id,consumer.enabled,consumer.max_get_messages_per_seccond], cb);
      },
      function(cb) {
...
BigQueueClustersAdmin.prototype.getTopicDataByCriteria = function(criteria, callback) {
  ...
  var topics_idx={};
  var query = "SELECT topic_id, ttl, cluster, enabled, max_new_messages_per_seccond FROM topics t "
      ...
      function(topics_db, cb) {
        ...
        topics_db.forEach(function(topic) {
          var data = {topic_id: topic.topic_id, cluster: topic.cluster,
            ttl: topic.ttl, enabled: topic.enabled == 1 ? true: false, max_new_messages_per_seccond: topic.max_new_messages_per_seccond,
            consumers: []};
          topics.push(data);
          ...
          data.forEach(function(consumer) {
            ...
            if(topic_data) {
              topic_data.consumers.push({consumer_id: consumer.consumer_id,
                ...
                max_get_messages_per_seccond: consumer.max_get_messages_per_seccond,
...
BigQueueClustersAdmin.prototype.getClusterStruct = function(cluster, callback) {
  var query = "SELECT t.topic_id t_topic_id, "+
    ...
    "t.ttl t_ttl, "+
    "t.enabled t_enabled, "+
    "t.max_new_messages_per_seccond t_max_new_messages_per_seccond, "+
    ...
    "c.max_get_messages_per_seccond c_max_get_messages_per_seccond, "+
      ...
      data.forEach(function(e) {
        if(topics[e["t_topic_id"]] == undefined) {
          topics[e["t_topic_id"]] = {
            ...
            "max_new_messages_per_seccond": e["t_max_new_messages_per_seccond"],
        ...
        if(e["c_consumer_id"]) {
          ...
          topics[e["t_topic_id"]].consumers.push({
            ...
            "max_get_messages_per_seccond": e["c_max_get_messages_per_seccond"]
          })
        }
      })
      var response = [];
...
BigQueueClustersAdmin.prototype.getConsumerByCriteria = function(criteria, callback) {
  ...
  var query = "SELECT c.consumer_id consumer_id, c.topic_id topic_id, "+
   ...
   "sum(fails) as fails, sum(processing) as processing, c.enabled, c.max_get_messages_per_seccond "+
   "FROM consumers c " +
   "LEFT OUTER JOIN stats s "+
   "ON(s.consumer = c.consumer_id AND s.topic = c.topic_id) ";
  var query_criteria = getCriteriaQuery(criteria,"c.");
  query+=query_criteria.query;
  query+=" GROUP BY c.consumer_id , c.topic_id, c.cluster,c.enabled, c.max_get_messages_per_seccond";
  this.mysqlCriticPool.getConnection(function(err, mysqlConn) {
    if(err) {
      if(mysqlConn) {
        mysqlConn.release();
      ...
      data.forEach( function(e){
        if(e.consumer_id)
          consumers.push({topic_id: e.topic_id, consumer_id: e.consumer_id, ttl: e.ttl,
                         cluster: e.cluster,
                         enabled: e.enabled == 1 ? true: false, max_get_messages_per_seccond: e.max_get_messages_per_seccond,
